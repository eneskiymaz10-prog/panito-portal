          ...
          <Table>
            ...
            <TableBody>
              {invoices?.map((inv) => (
                <TableRow key={inv.id}>
                  <TableCell>
                    <Link href={"/dashboard/accounting/invoices/" + inv.id} className="font-medium text-primary hover:underline">
                      {inv.invoice_number}
                    </Link>
                  </TableCell>
                  <TableCell>{(inv as any).orders?.order_number}</TableCell>
                  <TableCell>{(inv as any).orders?.profiles?.company_name || "-"}</TableCell>
                  ...
                  <TableCell>
                    {inv.status === "sent" && (
                      <Link href={"/dashboard/accounting/invoices/" + inv.id} className="text-sm text-primary hover:underline">
                        {t("confirmPayment")}
                      </Link>
                    )}
                  </TableCell>
